...
class _TranscriptorAppState extends State<TranscriptorWidget>
{
  ...
  Widget _buildIconButton(
    ...
    return new Padding(
      ...
      child: fab ? new FloatingActionButton(
        child: new Icon(icon),
        onPressed: onPress,
  ...
  Widget _buildTaskWidgets({Task task, VoidCallback onDelete, VoidCallback onComplete}) {
  ...
  Widget _buildTranscriptionBox({String text, VoidCallback onCancel, double width}) =>
      new Container(
          width: width,
  ...
  void _completeTaskHandler(Task completed) {
    setState(() {
      todos = todos.map((t) => completed == t ? (t..complete = true) : t).toList();
      ...
      case "onRecognitionComplete":
        {
          setState(() {
            if (todos.isEmpty) {
              ...
              // if user adds it before complete recognition,
              // just clear the transcription
              transcription = '';
            }
            else {
              transcription = call.arguments;
  ...
  void _showStatus(String action) {
    final label = "Task $action : ${incompleteTasks.length} left ${numArchived} archived";
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> blocks = [
      new Expanded(
          flex: 2,
          child: new ListView(
              children: incompleteTasks.map((t) => _buildTaskWidgets(
                  task: t,
                  onDelete: () => _deleteTaskHandler(t),
                  onComplete: () => _completeTaskHandler(t))).toList()
          ),
      ),
      _buildButtonBar()];
    if (isListening || transcription != '')
      blocks.insert(1, _buildTranscriptionBox(
          text: transcription,
